...
class PlugsCurdController extends PlugsBaseController
{
    ...
    public function create_curd(): Json
    {
        try {
            ...
            return $this->send(ErrorCode::THIRD_PART_ERROR, [11], $exception->getMessage());
        }
...
EOF;
    ...
    private function putModelFile(): bool
    {
        ...
        if (file_exists($fullPath)) {
            throw new Exception('模型已存在',ErrorCode::THIRD_PART_ERROR);
        }
        $create = file_put_contents($fullPath, $template);
        if ($create == false){
            throw new Exception('模型创建失败,请检查文件写入问题',ErrorCode::THIRD_PART_ERROR);
        }
        return true;
    }
    ...
    private function putControllerFile(): bool
    {
        ...
        if (file_exists($fullPath)) {
            throw new Exception('控制器已存在',ErrorCode::THIRD_PART_ERROR);
        }
        $create = file_put_contents($fullPath, $template);
        if ($create == false){
            throw new Exception('控制器创建失败,请检查文件写入问题',ErrorCode::THIRD_PART_ERROR);
        }
        return true;
    }
